...
public class DfcEmitterMenu extends AbstractContainerMenu {
    ...
    public DfcEmitterMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buffer) {
        this(containerId, playerInventory, getContainer(playerInventory, buffer.readBlockPos()), new SimpleContainerData(DfcEmitterBlockEntity.DATA_COUNT));
    }

    public DfcEmitterMenu(int containerId, Inventory playerInventory, Container container, ContainerData data) {
        super(HbmMenus.DFC_EMITTER.get(), containerId);
        checkContainerSize(container, 0);
        checkContainerDataCount(data, DfcEmitterBlockEntity.DATA_COUNT);
        this.container = container;
        this.data = data;
        this.blockPos = container instanceof DfcEmitterBlockEntity emitter ? emitter.getBlockPos() : BlockPos.ZERO;
    ...
    private static Container getContainer(Inventory playerInventory, BlockPos pos) {
        ...
        return blockEntity instanceof DfcEmitterBlockEntity emitter ? emitter : new SimpleContainer(0);
    }
}
