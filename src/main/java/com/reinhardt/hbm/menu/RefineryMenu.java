...
public class RefineryMenu extends AbstractContainerMenu {
    private static final int MACHINE_SLOT_COUNT = RefineryBlockEntity.SLOT_COUNT;
    ...
    public RefineryMenu(int containerId, Inventory playerInventory, RegistryFriendlyByteBuf buffer) {
        this(containerId, playerInventory, getContainer(playerInventory, buffer.readBlockPos()), new SimpleContainerData(RefineryBlockEntity.DATA_COUNT));
    }

    public RefineryMenu(int containerId, Inventory playerInventory, Container container, ContainerData data) {
        super(HbmMenus.REFINERY.get(), containerId);
        checkContainerSize(container, MACHINE_SLOT_COUNT);
        checkContainerDataCount(data, RefineryBlockEntity.DATA_COUNT);
        this.container = container;
        this.data = data;

        this.addSlot(new ValidatedSlot(container, RefineryBlockEntity.BATTERY_SLOT, 186, 72));
        this.addSlot(new ValidatedSlot(container, RefineryBlockEntity.INPUT_CONTAINER_SLOT, 8, 99));
        this.addSlot(new TakeOnlySlot(container, RefineryBlockEntity.INPUT_CONTAINER_RESULT_SLOT, 8, 119));
        this.addSlot(new ValidatedSlot(container, 3, 86, 99));
        this.addSlot(new TakeOnlySlot(container, 4, 86, 119));
        this.addSlot(new ValidatedSlot(container, 5, 106, 99));
        this.addSlot(new TakeOnlySlot(container, 6, 106, 119));
        this.addSlot(new ValidatedSlot(container, 7, 126, 99));
        this.addSlot(new TakeOnlySlot(container, 8, 126, 119));
        this.addSlot(new ValidatedSlot(container, 9, 146, 99));
        this.addSlot(new TakeOnlySlot(container, 10, 146, 119));
        this.addSlot(new TakeOnlySlot(container, RefineryBlockEntity.ITEM_OUTPUT_SLOT, 58, 119));
        this.addSlot(new ValidatedSlot(container, RefineryBlockEntity.FLUID_IDENTIFIER_SLOT, 186, 106));

        addPlayerInventory(playerInventory, 8, 150);
        addDataSlots(data);
    }
