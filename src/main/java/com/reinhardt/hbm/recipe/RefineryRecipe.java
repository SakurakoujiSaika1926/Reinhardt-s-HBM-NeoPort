...
public record RefineryRecipe(
        ...
        int power
) implements Recipe<RefineryRecipe.Input> {
    ...
    public RecipeSerializer<?> getSerializer() {
        return HbmRecipeTypes.REFINERY_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return HbmRecipeTypes.REFINERY.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<RefineryRecipe> {
        private static final MapCodec<RefineryRecipe> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
                Codec.STRING.optionalFieldOf("group", "").forGetter(RefineryRecipe::group),
                FluidIngredient.CODEC.fieldOf("input").forGetter(RefineryRecipe::input),
                FluidOutput.CODEC.listOf().fieldOf("outputs").forGetter(RefineryRecipe::outputs),
                ItemStack.STRICT_CODEC.optionalFieldOf("byproduct", ItemStack.EMPTY).forGetter(RefineryRecipe::byproduct),
                Codec.intRange(1, 1_000_000).optionalFieldOf("byproduct_interval", 100).forGetter(RefineryRecipe::byproductInterval),
                Codec.intRange(1, 1_000_000).optionalFieldOf("power", 5).forGetter(RefineryRecipe::power)
        ).apply(instance, RefineryRecipe::new));

        private static final StreamCodec<RegistryFriendlyByteBuf, RefineryRecipe> STREAM_CODEC = new StreamCodec<>() {
            @Override
            public RefineryRecipe decode(RegistryFriendlyByteBuf buffer) {
                ...
                return new RefineryRecipe(group, input, outputs, byproduct, byproductInterval, power);
            }

            @Override
            public void encode(RegistryFriendlyByteBuf buffer, RefineryRecipe recipe) {
