...
public record FractionTowerRecipe(
        String group,
        FluidIngredient input,
        FluidOutput output1,
        FluidOutput output2
) implements Recipe<FractionTowerRecipe.Input> {
    @Override
    ...
    public RecipeSerializer<?> getSerializer() {
        return HbmRecipeTypes.FRACTION_TOWER_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return HbmRecipeTypes.FRACTION_TOWER.get();
    }
    ...
    public static class Serializer implements RecipeSerializer<FractionTowerRecipe> {
        private static final MapCodec<FractionTowerRecipe> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
                Codec.STRING.optionalFieldOf("group", "").forGetter(FractionTowerRecipe::group),
                FluidIngredient.CODEC.fieldOf("input").forGetter(FractionTowerRecipe::input),
                FluidOutput.CODEC.fieldOf("output1").forGetter(FractionTowerRecipe::output1),
                FluidOutput.CODEC.fieldOf("output2").forGetter(FractionTowerRecipe::output2)
        ).apply(instance, FractionTowerRecipe::new));

        private static final StreamCodec<RegistryFriendlyByteBuf, FractionTowerRecipe> STREAM_CODEC = new StreamCodec<>() {
            @Override
            public FractionTowerRecipe decode(RegistryFriendlyByteBuf buffer) {
                ...
                return new FractionTowerRecipe(group, input, output1, output2);
            }

            @Override
            public void encode(RegistryFriendlyByteBuf buffer, FractionTowerRecipe recipe) {
        ...
        @Override
        public MapCodec<FractionTowerRecipe> codec() {
            return CODEC;
        }

        @Override
        public StreamCodec<RegistryFriendlyByteBuf, FractionTowerRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
}
