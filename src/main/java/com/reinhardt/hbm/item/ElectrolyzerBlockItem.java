...
public class ElectrolyzerBlockItem extends BlockItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        if (!(getBlock() instanceof ElectrolyzerBlock electrolyzerBlock)) {
        ...
        BlockPos corePos = ElectrolyzerBlock.legacyCorePos(clickedPos, facing);

        if (!ElectrolyzerBlock.canPlaceLegacyAt(level, clickedPos, facing, placeContext)) {
        ...
        BlockState state = electrolyzerBlock.defaultBlockState().setValue(ElectrolyzerBlock.FACING, facing);
        if (!level.setBlock(corePos, state, Block.UPDATE_ALL)) {
            return InteractionResult.FAIL;
        }

        electrolyzerBlock.placeLegacyDummies(level, corePos, facing, context.getPlayer());
