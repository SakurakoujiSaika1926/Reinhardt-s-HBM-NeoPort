...
public final class ChemicalPlantClientSounds {
    ...
    private ChemicalPlantClientSounds() {
    }

    public static void tick(ChemicalPlantBlockEntity chemicalPlant) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || minecraft.player == null || !chemicalPlant.isClientWorking()) {
            stop(chemicalPlant.getBlockPos());
            return;
        }

        BlockPos pos = chemicalPlant.getBlockPos().immutable();
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        private WorkingSound(BlockPos pos) {
            super(HbmSoundEvents.CHEMICAL_PLANT_OPERATE.get(), SoundSource.BLOCKS, SoundInstance.createUnseededRandom());
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof ChemicalPlantBlockEntity chemicalPlant) || !chemicalPlant.isClientWorking()) {
                requestStop();
                return;
            }
            ...
            this.volume = distance >= AUDIBLE_DISTANCE
                    ? 0.0F
                    : (float) (1.0D - distance / AUDIBLE_DISTANCE);
        }
