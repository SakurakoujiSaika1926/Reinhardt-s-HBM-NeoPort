...
public final class BatteryReddClientSounds {
    ...
    private BatteryReddClientSounds() {
    }

    public static void tick(BatteryReddBlockEntity battery) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || minecraft.player == null || battery.speed() <= 0.0F) {
            stop(battery.getBlockPos());
            return;
        }
        BlockPos pos = battery.getBlockPos().immutable();
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        private WorkingSound(BlockPos pos) {
            super(HbmSoundEvents.FENSU_HUM.get(), SoundSource.BLOCKS, SoundInstance.createUnseededRandom());
            this.pos = pos.immutable();
            this.x = pos.getX() + 0.5D;
            this.y = pos.getY() + 2.5D;
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof BatteryReddBlockEntity battery) || battery.speed() <= 0.0F) {
                requestStop();
                return;
            }
            ...
            float speed = battery.speed();
            this.volume = distance >= AUDIBLE_DISTANCE ? 0.0F : (float) (1.0D * (1.0D - distance / AUDIBLE_DISTANCE));
            this.pitch = 0.5F + speed / 15.0F * 1.5F;
        }
