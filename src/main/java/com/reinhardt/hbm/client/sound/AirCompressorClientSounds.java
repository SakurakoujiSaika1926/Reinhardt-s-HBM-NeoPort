...
public final class AirCompressorClientSounds {
    private static final double START_DISTANCE = 16.0D;
    private static final double AUDIBLE_DISTANCE = 16.0D;
    private static final int KEEP_ALIVE_TICKS = 20;
    private static final Map<BlockPos, WorkingSound> WORKING_SOUNDS = new HashMap<>();

    private AirCompressorClientSounds() {
    }

    public static void tick(AirCompressorBlockEntity compressor) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || minecraft.player == null || !compressor.running()) {
            stop(compressor.getBlockPos());
            return;
        }
        BlockPos pos = compressor.getBlockPos().immutable();
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        private WorkingSound(BlockPos pos) {
            super(HbmSoundEvents.ASSEMBLER_MOTOR.get(), SoundSource.BLOCKS, SoundInstance.createUnseededRandom());
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof AirCompressorBlockEntity compressor) || !compressor.running()) {
                requestStop();
                return;
            }
