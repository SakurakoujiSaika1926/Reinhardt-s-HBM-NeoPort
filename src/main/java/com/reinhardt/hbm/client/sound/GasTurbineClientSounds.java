...
public final class GasTurbineClientSounds {
    ...
    private GasTurbineClientSounds() {
    }

    public static void tick(GasTurbineBlockEntity turbine) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || minecraft.player == null || turbine.rpm() < 10 || turbine.state() == -1) {
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        private WorkingSound(BlockPos pos) {
            super(HbmSoundEvents.TURBINE_GAS_RUNNING.get(), SoundSource.BLOCKS, SoundInstance.createUnseededRandom());
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof GasTurbineBlockEntity turbine) || turbine.rpm() < 10 || turbine.state() == -1) {
                requestStop();
                return;
            }
            ...
            this.volume = distance >= AUDIBLE_DISTANCE ? 0.0F : (float) (2.0F * (1.0D - distance / AUDIBLE_DISTANCE));
            this.pitch = (float) (0.55D + 0.1D * turbine.rpm() / 10.0D);
        }
