...
/** Client counterpart of TileEntityMachinePyroOven#createAudioLoop. */
public final class PyroOvenClientSounds {
    ...
    private PyroOvenClientSounds() {
    }

    public static void tick(LegacyMachineBlockEntity oven) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null || minecraft.player == null || !oven.pyroProgressing()) {
            stop(oven.getBlockPos());
            return;
        }
        BlockPos pos = oven.getBlockPos().immutable();
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        private WorkingSound(BlockPos pos) {
            super(HbmSoundEvents.PYRO_OVEN_OPERATE.get(), SoundSource.BLOCKS, SoundInstance.createUnseededRandom());
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof LegacyMachineBlockEntity oven)
                    || !oven.machineId().equals("machine_pyrooven") || !oven.pyroProgressing()) {
                requestStop();
                return;
            }
