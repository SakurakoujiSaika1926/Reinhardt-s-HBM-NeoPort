...
public final class LargeTurbineClientSounds {
    ...
    private LargeTurbineClientSounds() {
    }

    public static void tick(LargeTurbineBlockEntity turbine) {
    ...
    private static final class WorkingSound extends AbstractTickableSoundInstance {
        ...
        public void tick() {
            ...
            if (!(blockEntity instanceof LargeTurbineBlockEntity turbine) || turbine.fanAcceleration() <= 0.0F) {
                requestStop();
                return;
            }
            ...
            float speed = Math.min(1.0F, turbine.fanAcceleration() / 15.0F);
            this.volume = distance >= AUDIBLE_DISTANCE ? 0.0F : (float) (0.4F * speed * (1.0D - distance / AUDIBLE_DISTANCE));
