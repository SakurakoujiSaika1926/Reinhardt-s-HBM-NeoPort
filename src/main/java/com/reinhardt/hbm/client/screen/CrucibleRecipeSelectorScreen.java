...
public class CrucibleRecipeSelectorScreen extends Screen {
    ...
    private final List<RecipeHolder<CrucibleRecipe>> originRecipes;
    private final List<RecipeHolder<CrucibleRecipe>> recipes = new ArrayList<>();
    ...
    public CrucibleRecipeSelectorScreen(CrucibleScreen previousScreen, CrucibleMenu menu) {
        super(Component.translatable("container.reinhardtshbm.machine_crucible.recipe_selector"));
        this.previousScreen = previousScreen;
        this.menu = menu;
        this.originRecipes = List.copyOf(menu.crucibleRecipes());
    ...
    private void renderSelector(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        for (int recipeIndex = this.pageIndex * 8; recipeIndex < this.pageIndex * 8 + 40 && recipeIndex < this.recipes.size(); recipeIndex++) {
            ...
            RecipeHolder<CrucibleRecipe> holder = this.recipes.get(recipeIndex);
        ...
        RecipeHolder<CrucibleRecipe> selected = selectedRecipe();
        if (selected != null) {
    ...
    private void renderRecipeIcon(GuiGraphics guiGraphics, RecipeHolder<CrucibleRecipe> holder, int x, int y) {
        ItemStack icon = holder.value().icon().copy();
    ...
    @Nullable
    private RecipeHolder<CrucibleRecipe> selectedRecipe() {
        if (this.selectedRecipeId == null) {
            return null;
        }
        for (RecipeHolder<CrucibleRecipe> holder : this.originRecipes) {
    ...
    public static List<Component> describeRecipe(RecipeHolder<CrucibleRecipe> holder) {
        CrucibleRecipe recipe = holder.value();
        List<Component> lines = new ArrayList<>();
        lines.add(recipeTitle(recipe).withStyle(ChatFormatting.YELLOW));
        lines.add(Component.translatable("info.reinhardtshbm.template_in").withStyle(ChatFormatting.BOLD));
        for (CrucibleRecipe.MaterialIngredient ingredient : recipe.input()) {
            lines.add(materialLine(ingredient));
        }
    ...
    private static MutableComponent recipeTitle(CrucibleRecipe recipe) {
        if (!recipe.group().isBlank()) {
            return Component.translatable(recipe.group());
        }
        return recipe.icon().getHoverName().copy();
    }

    private static Component materialLine(CrucibleRecipe.MaterialIngredient ingredient) {
        return Component.literal(" - ")
                .withStyle(ChatFormatting.GRAY)
                .append(Component.translatable(ingredient.material().translationKey()))
                .append(Component.literal(": "))
                .append(ScrapsItem.formatAmountComponent(ingredient.amount()));
    }
