...
public class LargeTurbineScreen extends AbstractContainerScreen<LargeTurbineMenu> {
    private static final ResourceLocation TEXTURE = ReinhardtsHBM.id("textures/gui/generators/gui_turbine_large.png");

    public LargeTurbineScreen(LargeTurbineMenu menu, Inventory playerInventory, Component title) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (isHovering(123, 35, 7, 34, mouseX, mouseY)) {
            guiGraphics.renderComponentTooltip(this.font, List.of(Component.translatable(
                    ...
                    this.menu.power(),
                    LargeTurbineBlockEntity.ENERGY_CAPACITY
            )), mouseX, mouseY);
        }
        if (this.menu.outputFluid().isNone() && isHovering(-16, 68, 16, 16, mouseX, mouseY)) {
            guiGraphics.renderComponentTooltip(this.font, List.of(Component.translatable("desc.reinhardtshbm.errorfluid")), mouseX, mouseY);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
        ...
        if (this.menu.outputFluid().isNone()) {
            guiGraphics.blit(TEXTURE, this.leftPos - 16, this.topPos + 68, 176, 56, 16, 16);
        }
    }
