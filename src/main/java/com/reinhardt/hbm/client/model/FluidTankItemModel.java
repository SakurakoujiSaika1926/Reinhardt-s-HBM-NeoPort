...
public final class FluidTankItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();

    private final BakedModel delegate;

    private FluidTankItemModel(BakedModel delegate) {
        this.delegate = delegate;
    }

    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        ModelResourceLocation location = new ModelResourceLocation(ReinhardtsHBM.id("machine_fluidtank"), ModelResourceLocation.INVENTORY_VARIANT);
        BakedModel model = models.get(location);
        if (model != null && !(model instanceof FluidTankItemModel)) {
            models.put(location, new FluidTankItemModel(model));
            ReinhardtsHBM.LOGGER.info("Installed dynamic item renderer hook for Fluid Tank item model");
        }
    }
