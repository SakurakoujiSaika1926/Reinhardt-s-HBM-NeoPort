...
public final class SteamEngineItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();

    private final BakedModel delegate;

    private SteamEngineItemModel(BakedModel delegate) {
        this.delegate = delegate;
    }

    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        ModelResourceLocation location = new ModelResourceLocation(ReinhardtsHBM.id("machine_steam_engine"), ModelResourceLocation.INVENTORY_VARIANT);
        BakedModel model = models.get(location);
        if (model != null && !(model instanceof SteamEngineItemModel)) {
            models.put(location, new SteamEngineItemModel(model));
            ReinhardtsHBM.LOGGER.info("Installed animated item renderer hook for Steam Engine item model");
        }
    }
