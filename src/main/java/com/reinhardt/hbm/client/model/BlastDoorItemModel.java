...
public final class BlastDoorItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();
    private final BakedModel delegate;

    private BlastDoorItemModel(BakedModel delegate) {
    ...
    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        for (ModelResourceLocation location : List.copyOf(models.keySet())) {
            if (!isBlastDoorInventory(location)) {
                continue;
            }
            BakedModel model = models.get(location);
            if (model != null && !(model instanceof BlastDoorItemModel)) {
                models.put(location, new BlastDoorItemModel(model));
            }
        }
    }
    ...
    private static boolean isBlastDoorInventory(ModelResourceLocation location) {
        return ModelResourceLocation.INVENTORY_VARIANT.equals(location.variant())
                ...
                && location.id().getPath().equals("blast_door");
    }
