...
public final class FluidDuctNeoBakedModel implements IDynamicBakedModel {
    private static final Set<ResourceLocation> BLOCK_IDS = Set.of(
            ReinhardtsHBM.id("fluid_duct_mk2"),
            ReinhardtsHBM.id("fluid_duct_neo")
    );
    private static final ResourceLocation OBJ_LOCATION = ReinhardtsHBM.id("models/obj/fluid_pipe/pipe_neo.obj");
    private static final ResourceLocation BASE_TEXTURE = ReinhardtsHBM.id("block/fluid_duct_mk2");
    private static final ResourceLocation OVERLAY_TEXTURE = ReinhardtsHBM.id("block/fluid_duct_mk2_overlay");
    private static final Transform WORLD_TRANSFORM = new Transform(1.0F, 0.0F, 0.0F, 0.0F, 0.0F, true);
    private static final Transform ITEM_TRANSFORM = new Transform(1.0F, 0.5F, 0.0F, 0.5F, (float) Math.PI, false);
    ...
    private FluidDuctNeoBakedModel(BakedModel fallbackModel, ObjMesh mesh, TextureAtlasSprite baseSprite, TextureAtlasSprite overlaySprite, boolean forBlock) {
        ...
        if (forBlock) {
            ...
            this.itemQuads = bakeWithOverlay(Set.of("pX", "nX", "pZ", "nZ"), ITEM_TRANSFORM);
        }
    }
    ...
    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models, Function<Material, TextureAtlasSprite> textureGetter) {
        List<ModelResourceLocation> blockLocations = models.keySet().stream()
                .filter(FluidDuctNeoBakedModel::isBlockModel)
                .toList();
        List<ModelResourceLocation> itemLocations = BLOCK_IDS.stream()
                .map(id -> new ModelResourceLocation(id, ModelResourceLocation.INVENTORY_VARIANT))
                .toList();
        BakedModel blockFallback = blockLocations.stream()
                .map(models::get)
                .filter(Objects::nonNull)
                .findFirst()
                .orElseGet(() -> itemLocations.stream()
                        .map(models::get)
                        .filter(Objects::nonNull)
                        .findFirst()
                        .orElse(null));
        if (blockFallback == null) {
            ReinhardtsHBM.LOGGER.warn("Unable to install universal fluid duct OBJ model: no baked model was found");
            return;
        }

        ObjMesh mesh = ObjMesh.load(OBJ_LOCATION);
        if (mesh.isEmpty()) {
            ReinhardtsHBM.LOGGER.warn("Unable to install universal fluid duct OBJ model: {} did not load", OBJ_LOCATION);
            return;
        }

        TextureAtlasSprite base = textureGetter.apply(new Material(TextureAtlas.LOCATION_BLOCKS, BASE_TEXTURE));
        TextureAtlasSprite overlay = textureGetter.apply(new Material(TextureAtlas.LOCATION_BLOCKS, OVERLAY_TEXTURE));
        FluidDuctNeoBakedModel blockModel = new FluidDuctNeoBakedModel(blockFallback, mesh, base, overlay, true);
        for (ModelResourceLocation location : blockLocations) {
            models.put(location, blockModel);
        }

        int itemCount = 0;
        for (ModelResourceLocation itemLocation : itemLocations) {
            if (models.containsKey(itemLocation)) {
                BakedModel itemFallback = models.getOrDefault(itemLocation, blockFallback);
                models.put(itemLocation, new FluidDuctNeoBakedModel(itemFallback, mesh, base, overlay, false));
                itemCount++;
            }
        }
        ReinhardtsHBM.LOGGER.info("Installed universal fluid duct OBJ model for {} baked block variants and {} item models", blockLocations.size(), itemCount);
    }
    ...
    private List<BakedQuad>[] precomputeBlockQuads() {
        ...
        for (int mask = 0; mask < quads.length; mask++) {
            quads[mask] = bakeWithOverlay(partsFor(mask), WORLD_TRANSFORM);
        }
        return quads;
    }

    private List<BakedQuad> bakeWithOverlay(Set<String> parts, Transform transform) {
        List<ObjFace> faces = this.mesh.faces(parts);
        List<BakedQuad> quads = new ArrayList<>(faces.size() * 2);
        for (ObjFace face : faces) {
            quads.add(bakeFace(face, this.baseSprite, BlockElementTint.NONE, transform));
            quads.add(bakeFace(face, this.overlaySprite, BlockElementTint.FLUID, transform));
        }
        return List.copyOf(quads);
    }

    private BakedQuad bakeFace(ObjFace face, TextureAtlasSprite sprite, int tintIndex, Transform transform) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            baker.setUv(sprite.getU(uv.u()), sprite.getV(uv.v()));
            baker.setLight(0);
    ...
    private static boolean isBlockModel(ModelResourceLocation location) {
        return BLOCK_IDS.contains(location.id()) && !ModelResourceLocation.INVENTORY_VARIANT.equals(location.variant());
    }
    ...
    private static final class ObjMesh {
        ...
        private static ObjMesh load(ResourceLocation location) {
            ...
            try (InputStream stream = FluidDuctNeoBakedModel.class.getClassLoader().getResourceAsStream(resourcePath)) {
                if (stream == null) {
                    return EMPTY;
                }
                return parse(stream);
