...
public final class PurexItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();

    private final BakedModel delegate;

    private PurexItemModel(BakedModel delegate) {
        this.delegate = delegate;
    }

    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        ModelResourceLocation location = new ModelResourceLocation(
                ReinhardtsHBM.id("machine_purex"),
                ModelResourceLocation.INVENTORY_VARIANT
        );
        BakedModel model = models.get(location);
        if (model != null && !(model instanceof PurexItemModel)) {
            models.put(location, new PurexItemModel(model));
            ReinhardtsHBM.LOGGER.info("Installed custom item renderer hook for the PUREX item model");
        }
    }
