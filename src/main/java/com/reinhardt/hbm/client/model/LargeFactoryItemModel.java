...
public final class LargeFactoryItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();

    private final BakedModel delegate;

    private LargeFactoryItemModel(BakedModel delegate) {
    ...
    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        ...
        for (ModelResourceLocation location : List.copyOf(models.keySet())) {
            if (!isLargeFactoryInventory(location)) {
                continue;
            }
            BakedModel model = models.get(location);
            if (model != null && !(model instanceof LargeFactoryItemModel)) {
                models.put(location, new LargeFactoryItemModel(model));
                replaced++;
            }
        }
        if (replaced > 0) {
            ReinhardtsHBM.LOGGER.info("Installed custom item renderer hooks for {} large factory item models", replaced);
        }
    }
    ...
    private static boolean isLargeFactoryInventory(ModelResourceLocation location) {
        ...
        return path.equals("machine_assembly_factory") || path.equals("machine_chemical_factory");
    }
