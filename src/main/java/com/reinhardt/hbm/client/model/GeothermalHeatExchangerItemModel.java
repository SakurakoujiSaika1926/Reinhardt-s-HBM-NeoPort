...
public final class GeothermalHeatExchangerItemModel implements IDynamicBakedModel {
    private static final ItemTransforms ITEM_TRANSFORMS = itemTransforms();
    private final BakedModel delegate;

    private GeothermalHeatExchangerItemModel(BakedModel delegate) {
        this.delegate = delegate;
    }

    public static void replaceModels(Map<ModelResourceLocation, BakedModel> models) {
        ModelResourceLocation location = new ModelResourceLocation(
                ReinhardtsHBM.id("machine_hephaestus"), ModelResourceLocation.INVENTORY_VARIANT);
        BakedModel model = models.get(location);
        if (model != null && !(model instanceof GeothermalHeatExchangerItemModel)) {
            models.put(location, new GeothermalHeatExchangerItemModel(model));
            ReinhardtsHBM.LOGGER.info("Installed item renderer hook for Geothermal Heat Exchanger");
        }
    }
    ...
    @Override public ItemTransforms getTransforms() { return ITEM_TRANSFORMS; }
    ...
    private static ItemTransform transform(float rotX, float rotY, float rotZ,
        ...
        return new ItemTransform(new Vector3f(rotX, rotY, rotZ),
                new Vector3f(translateX, translateY, translateZ), new Vector3f(scale, scale, scale));
    }
}
